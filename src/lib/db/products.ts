 ...
 */
export async function scanAllProducts(): Promise<Product[]> {
  try {
    const items: Product[] = [];
...
export async function queryByCategory(categorySlug: string): Promise<Product[]> {
  try {
    const items: Product[] = [];
    let ExclusiveStartKey: Record<string, unknown> | undefined;
...
/** Number of products currently assigned to a category — used by the admin
 *  category screens to keep the denormalized `Category.productCount` field
 *  accurate and to block deleting a category that's still in use. */
export async function getCategoryProductCount(categorySlug: string): Promise<number> {
  const products = await queryByCategory(categorySlug);
  return products.length;
}

/**
 * Admin catalog listing: unlike `listProducts` (storefront-facing, returns
 * trimmed `ProductSummary`s), this returns full `Product` records — the
 * admin table and edit forms need fields like `description` and `variants`
 * that the public summary intentionally omits.
 */
export interface AdminProductFilters {
  ...
  sort?: "newest" | "name_asc" | "price_asc" | "price_desc" | "stock_asc" | "stock_desc";
  page?: number;
  pageSize?: number;
}
...
export async function listProductsAdmin(
  ...
  if (q) {
    const needle = q.toLowerCase().trim();
    filtered = filtered.filter((p) => {
      const haystack = `${p.name} ${p.productId} ${p.slug} ${p.tags.join(" ")}`.toLowerCase();
      return haystack.includes(needle);
    });
  }
...
/** Admin-only: sets stock to an absolute value (not a delta), e.g. after a
 *  restock or inventory correction. Distinct from `decrementStock`, which is
 *  the storefront checkout path and only ever moves stock down by a delta. */
export async function setProductStock(productId: string, stock: number): Promise<Product> {
  ...
  try {
    const result = await docClient.send(
      new UpdateCommand({
        ...
        ExpressionAttributeValues: { ":stock": stock, ":now": new Date().toISOString() },
